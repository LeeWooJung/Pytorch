...
class CNN(nn.Module):
    ...
    def __init__(self, k, e_char, e_word, batch_size):

        self.Conv1D = nn.Conv1d(in_channels = e_char, output_channels = e_word,
				kernel_size = k, bias = True)
        self.relu = nn.ReLU()

	def __forward__(self, x_reshaped):
        ...
        x_conv_out, _ = torch.max(self.relut(x_conv),2)
        # x_conv_out : [e_word, batch_size]
        
        return x_conv_out.permute(1,0)
